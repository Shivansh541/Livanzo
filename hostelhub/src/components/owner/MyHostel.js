...
const MyHostel = () => {
  ...
  useEffect(() => {
    const fetchMyHostels = async () => {
      const res = await fetch('https://livanzo.onrender.com/api/hostel/myHostels', {
        method: 'GET',
        headers: {
        ...
        <div className="hostel-grid">
          {myHostels.map((h) => (
            <div key={h._id} className="hostel-card">
              <img onClick={() => handleClick(h._id)} src={h.images[0].startsWith('http')?h.images[0]:`https://livanzo.onrender.com${h.images[0]}`} alt="No img" />
              <h3>{h.name}</h3>
              <p>{h.description}</p>
              <p>Rent: ₹{h.rent}</p>
